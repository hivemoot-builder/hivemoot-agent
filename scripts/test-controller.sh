...
run_provider_secret_staging_pre_trap_cleanup_case() {
  local repo_root="$1"
  local case_dir="$2"
  local provider_secret_source="${case_dir}/secrets/openai-api-key"
  local missing_file="${case_dir}/secrets/google-api-key-does-not-exist"
  ...
  # OPENAI_API_KEY_FILE points to a valid file (will be staged first),
  # GOOGLE_API_KEY_FILE points to a missing file (staging fails mid-way).
  # With trap installed before staging, cleanup must run even on this early exit.
  if env -i \
    PATH="${case_dir}/mock-bin:${PATH}" \
    HOME="${case_dir}/home" \
    MOCK_DOCKER_STATE_DIR="${case_dir}/mock-state" \
    ...
    AGENT_GITHUB_TOKEN_01="token-1" \
    OPENAI_API_KEY_FILE="${provider_secret_source}" \
    GOOGLE_API_KEY_FILE="${missing_file}" \
    AGENT_TIMEOUT_SECONDS="120" \
    PERIODIC_INTERVAL_SECS="60" \
    PERIODIC_JITTER_SECS="0" \
    bash "${repo_root}/scripts/controller.sh"; then
    fail "controller succeeded unexpectedly in pre-trap staging failure case"
  fi

  if find "${case_dir}/workspace/controller-secrets" -type f -print -quit 2>/dev/null | grep -q .; then
    fail "staged provider secrets were not cleaned up after startup failure"
  fi

  echo "PASS: staged provider secrets are cleaned up when startup fails mid-staging"
}
...
run_provider_secret_staging_pre_trap_cleanup_case "$repo_root" "${tmpdir}/provider-secret-staging-pre-trap"
